...
# Implement the TreeBoster class
class TreeBooster():
    def __init__(self, X, g, h, params, max_depth,  idxs=None):
        self.params = params 
        self.max_depth = max_depth 
        assert self.max_depth >= 0, "max_depth mus be non-negative"
    ...
    @property
    def is_leaf(self): return self.best_score_so_far == 0.0

    def _find_better_split(self, feature_idx):
        x = self.X.values[self.idxs, feature_idx]
        g, h = self.g[self.idxs], self.h[self.idxs]
        sorted_idx = np.argsort(x)
        sort_g, sort_h, sort_x = g[sorted_idx], h[sorted_idx], x[sorted_idx]
        sum_g, sum_h = sort_g.sum(), sort_h.sum()
        sum_g_left, sum_h_right = sum_g, sum_h
